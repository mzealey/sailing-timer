...
const calcSecsFromStart = (startTime) => startTime > 0 ? Math.floor((Date.now() - startTime) / 1000) : NaN;
...
export function RaceTimer() {
    ...
    if (secsFromStart >= 0)
        return <>
                Elapsed: {formattedTime(secsFromStart)}
            </>;
    else if (secsFromStart < 0)
        return <>
                To start of race: {formattedTime(-secsFromStart)}
            </>;
}
